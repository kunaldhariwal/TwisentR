...
shinyServer(function(input, output, session) {
  ...
  TweetFrame<-function(twtList)
  {
	...
	{
		neg.words<<-c(neg.words, 'Fight', 'fighting', 'wtf', 'arrest', 'no', 'not','what the fuck','fuck','hell')
   ...
   score.sentiment <- function(sentences, pos.words, neg.words, .progress='none')
	...
	output$word<-renderPlot({wordcloud(text_word(), random.order=F,max.words=100, col=rainbow(100), scale=c(2.5,1.5))		
				})
	
	#HISTOGRAM

	output$histPos<- renderPlot({hist(table_final()$Positive, col=terrain.colors(10), main = "Histogram of Positive Sentiment", xlab = "Positive Score") })
	output$histNeg<- renderPlot({hist(table_final()$Negative, col=heat.colors(10), main = "Histogram of Negative Sentiment", xlab = "Negative Score") })
	output$histScore<- renderPlot({hist(table_final()$Score, col=topo.colors(10), main = "Histogram of Score", xlab = "Overall Score")})
	...
	library(plotrix)
	output$piechart<-renderPlot({pie3D(slices(), labels = labels, col=rainbow(length(labels)),explode=0.1, main="Sentiment Analysis") })
	...
	output$tweetersplot<-renderPlot ( barplot(head(d()$Tweets, 20),  names=head(d()$User, 20), las=2, horiz=F, main="Top 20: Tweets per User", col=8,font.axis=2, col.axis="blue" , cex.axis=1.5) )
	...
	p<- reactive ({ p = ggplot(dat2(), aes(x = tag, y = freq)) + geom_bar(stat="identity", fill = "green")
	p + coord_flip() + labs(title = "Frequency of HashTags in the tweets of the user") })
